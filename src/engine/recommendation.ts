...
export function recommendHeroes(
  ...
  enemies.forEach(e => {
    ...
    enemyLaneBully += (e.tags['lane_bully'] || 0);
  });
  ...
  const heroesToEvaluate = HEROES.filter(
    h => (h.roleWeights?.[desiredRole] || (h.roles.includes(desiredRole) ? 1 : 0)) > 0.05 &&
         !allies.some(a => a.id === h.id) &&
  ...
  const evaluatedHeroes = heroesToEvaluate.map(hero => {
    ...
    if (roleWeight < 0.4) {
      score -= 50.0; // Massive penalty for off-meta roles
    ...
    if (highSustainActive) {
      ...
      if (heroAntiHeal > 0) {
        const boost = heroAntiHeal * enemySustain * 15.0;
        score += boost;
    ...
    if (highPassiveActive) {
      ...
      if (heroBreak > 0) {
        const boost = heroBreak * enemyPassive * 10.0;
        score += boost;
    ...
    if (allies.length > 0) {
      ...
      // Early Game / Laning Phase Safeguard (Pudge 0 armor / Weak laners vs heavy bullies)
      if ((desiredRole === 'Offlane' || desiredRole === 'Mid' || desiredRole === 'Carry') && enemyLaneBully > 1.2) {
        if ((hTags['weak_laner'] || 0) > 0 || ((hTags['melee'] || 0) > 0 && (hTags['durable'] || 0) < 0.7 && (hTags['escape'] || 0) < 0.5)) {
  ...
  if (enemySustain > 1.8) {
    ...
    if (!hasAntiHealAlly && !hasAntiHealRec) {
      warnings.push(
        `⚠️ **High Sustain Threat**: Enemies rely extremely heavily on health regeneration and healing (e.g., Alchemist, Huskar). If Ancient Apparition or Doom are banned, Cores **MUST** build **Eye of Skadi** or **Shiva's Guard**, and Supports **MUST** purchase **Spirit Vessel** immediately.`
      );
